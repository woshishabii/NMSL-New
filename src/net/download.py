...
def download(logger, lang, url: str, file_name: str, retry_times: int = 3, each_size=16*MB) -> None:
    '''
    根据文件直链和文件名下载文件
    ...
    logger.log(message=f'{lang.start_download_part}：{len(parts)}')
    # 创建进度条
    bar = tqdm(total=file_size, desc=f'{lang.download_progress_bar}：{file_name}')
    for part in parts:
        start, end = part
